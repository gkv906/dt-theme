...
function _initDeskSlider(id, containerId, progressId, intervalMs) {
  ...
  if (!cards.length) return;
  // Wait one frame so layout is complete and card widths are accurate
  requestAnimationFrame(() => {
...
function _stepDeskSlider(id, containerId, progressId, dir) {
  ...
  _updateDeskProgress(id, progressId);
  // Restart timer so manual click resets interval
  if (s.timer) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Using scroll-based approach; init after layout settles
  requestAnimationFrame(() => {
    setTimeout(() => {
